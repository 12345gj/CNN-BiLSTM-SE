...
# 5. 构建 BiLSTM 模型
model = Sequential([
    Bidirectional(LSTM(128, return_sequences=False), input_shape=(seq_length, 6)),
...
print("\n=== 正在训练 BiLSTM 模型 ===")
model.fit(X_train, y_train, epochs=100, batch_size=32, class_weight=class_weights, validation_data=(X_test, y_test), verbose=0)

y_pred = np.argmax(model.predict(X_test), axis=1)
print(f"BiLSTM 准确率: {accuracy_score(y_test, y_pred):.4f}")
